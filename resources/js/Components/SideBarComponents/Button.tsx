...
export default function Button({iconName, text, selected, routeName}: { iconName: string, text: string, selected: boolean, routeName: string }) {
    return (
        ...
        >
            <Icons className={`${selected ? 'fill-text-dark-primary dark:fill-text-dark-primary' : 'fill-text-light-primary dark:fill-text-dark-primary group-hover:fill-text-dark-primary transition-colors duration-150'}`} iconName={iconName} />
            <p>{text}</p>
        </Link>
    );
}
