...
export default function TextInput({ id, logo, placeholder, onClickDelete }: { id: string, logo: string, placeholder: string, onClickDelete: any }) {
    ...
    return (
        <div className="relative w-full cursor-text">
            ...
            <input id={`document-input-${id}`} className="relative text-sm md:text-base text-text-light-disabled dark:text-text-dark-disabled w-full p-3 px-12 rounded-xl bg-transparent border-2 border-background-light-neutral dark:border-background-dark-neutral focus:border-primary-main focus:dark:border-primary-main transition-colors duration-150 focus:ring-0" type="search" defaultValue={placeholder} />
